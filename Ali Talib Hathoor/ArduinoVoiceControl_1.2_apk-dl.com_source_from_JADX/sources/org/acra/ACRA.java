...
public class ACRA {
    ...
    static class C03551 implements OnSharedPreferenceChangeListener {
        C03551() {
        }
    ...
    public static void init(Application app) {
        ...
        if (mReportsCrashes == null) {
            log.mo1607e(LOG_TAG, "ACRA#init called but no ReportsCrashes annotation on Application " + mApplication.getPackageName());
            return;
        }
        SharedPreferences prefs = getACRASharedPreferences();
        try {
            checkCrashResources();
            log.mo1605d(LOG_TAG, "ACRA is enabled for " + mApplication.getPackageName() + ", intializing...");
            ...
            log.mo1615w(LOG_TAG, "Error : ", e);
        }
        mPrefListener = new C03551();
    ...
    public static ACRAConfiguration getConfig() {
        if (configProxy == null) {
            if (mApplication == null) {
                log.mo1614w(LOG_TAG, "Calling ACRA.getConfig() before ACRA.init() gives you an empty configuration instance. You might prefer calling ACRA.getNewDefaultConfig(Application) to get an instance with default values taken from a @ReportsCrashes annotation.");
            }
